...
import { ChevronLeft, ChevronRight, Plus, LogOut, Calendar as CalendarIcon, Sparkles } from 'lucide-react';
...
export const CalendarPage: React.FC = () => {
  ...
  }, [accessToken, selectedCalendarId, rangeStart, rangeEnd]);
  ...
  const handleSaveEvent = async (eventData: EventInput, eventId?: number) => {
    ...
    if (selectedCalendarId) {
      if (eventId) {
        await api.updateSharedCalendarEvent(accessToken, selectedCalendarId, eventId, eventData);
      } else {
        await api.createSharedCalendarEvent(accessToken, selectedCalendarId, eventData);
      }
    } else {
      if (eventId) {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-100 overflow-hidden font-sans select-none">
      ...
      {/* Main Viewport Container: Fixed Height, Independent Child Scrolling */}
      <main className="flex-1 min-h-0 p-4 sm:p-5 flex flex-col overflow-hidden">
        ...
        {/* 2-Column Split: Static Calendar Matrix (Left 2 cols) & Dedicated Scrollable Chat (Right 1 col) */}
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-5 overflow-hidden">
          {/* Left Column: Fixed / Static Calendar Matrix */}
          ...
          {/* Right Column: AI Chat Panel - Independently scrollable messages area */}
